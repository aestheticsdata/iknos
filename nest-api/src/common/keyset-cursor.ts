/**
 * The pagination cursor for a keyed list — the issues list (IKN-14) and the alerts list (IKN-10).
 *
 ...
 * These lists are orderable by volume as well, where the key is a count — an integer that is not
 * an instant and would have to be dressed as one to fit. Their ids are plain `INT`s too, so the
 * BigInt handling there buys nothing. Two small functions that say what they carry beat one that
 * lies about it.
 *
...
export function encodeKeysetCursor(key: number, id: number): string {
 ...
 */
export function decodeKeysetCursor(raw: string): { key: number; id: number } | null {
