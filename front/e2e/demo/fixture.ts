 ...
 *
 * Iknos cannot fail that way today: its fonts come through
 ...
 *
 * Next's three only: Iknos has no TanStack and no third-party toaster. Its own toast host is
 * deliberately absent — a toast is the monitor talking, not the toolchain, and the take is
 * read-only anyway.
 */
...
export const test = base.extend<{ demo: Demo }>({
  demo: async ({ page }, use) => {
    ...
    await page.close();
    if (video) await video.saveAs(join(OUT_DIR, "iknos-demo.webm"));
